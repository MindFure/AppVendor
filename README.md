...
1. **Отказоустойчивая база данных (Авто-Failover):** Кластер СУБД управляется Patroni. В случае падения лидера (`pg-node1`), etcd фиксирует изменение состояния, и `pg-node2` автоматически становится лидером. HAProxy мгновенно перехватывает трафик и направляет его на выжившую ноду. Zero-downtime для пользователей.
2. **Идемпотентный деплой:** Вся система разворачивается с нуля одной командой `ansible-playbook`.
3. **Автоматизация эксплуатации:** * Ночные бэкапы БД без остановки контейнеров с автоматической очисткой архивов старше 7 дней.
   * Демон `network_watchdog` для непрерывного мониторинга состояния сети и логирования перезагрузок сервера.
4. **Безопасность:** Отсутствие хардкода секретов в репозитории (используется `.gitignore` и локальные `.env` файлы), строгие права доступа (группа `docker`).
...
3. **Запустите автоматический деплой через Ansible:**
   ```bash
...
* **Интернет-магазин:** `http://<IP_сервера>`
